...
var arrayTestCases = []testCase{
	{
		...
		sch:  new(elem),
		j:    `["foo"]`,
		jErr: `hjson.arrayType.UnmarshalJSON: array schema is nil for non-empty array`,
	}, {
		name: "ExtraElemsInSchema",
		sch:  &elem{Type: elemTypeArray, Items: []*elem{stringSchema, stringSchema}},
		j:    `["foo"]`,
		jErr: `hjson.arrayType.UnmarshalJSON: 2 elements in schema, 1 in total`,
	}, {
		name: "ExtraElemsInArray",
		sch:  &elem{Type: elemTypeArray, Items: []*elem{stringSchema}},
		j:    `["foo", "bar"]`,
		jErr: `hjson.arrayType.UnmarshalJSON: 1 elements in schema, 2 in total`,
	},
}

func TestArray(t *testing.T) {
	t.Parallel()
	testJSON(t, arrayTestCases, func() hjsontype { return new(arrayType) })
}

func FuzzArray(f *testing.F) {
	fuzzJSON(f, arrayTestCases, func() hjsontype { return new(arrayType) })
}

func BenchmarkArray(b *testing.B) {
	benchmark(b, arrayTestCases, func() hjsontype { return new(arrayType) })
}
