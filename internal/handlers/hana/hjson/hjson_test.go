...
func testJSON(t *testing.T, testCases []testCase, newFunc func() hjsontype) {
	for _, tc := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			t.Run("Marshal", func(t *testing.T) {
				...
				actualJ, err := MarshalSingleValue(fromhjson(tc.v))
				require.NoError(t, err)
...
func fuzzJSON(f *testing.F, testCases []testCase, newFunc func() hjsontype) {
	for _, tc := range testCases {
	...
	f.Fuzz(func(t *testing.T, j, jsch string) {
		...
		{
			d, ok := fromhjson(v).(*types.Document)
			if !ok {
				t.Skip()
			}
...
func benchmark(b *testing.B, testCases []testCase, newFunc func() hjsontype) {
	for _, tc := range testCases {
		tc := tc
...
func unmarshalJSON(v hjsontype, tc *testCase) error {
	var err error
	switch v := v.(type) {
	case *documentType:
